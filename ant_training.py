...
ant_data["choice_mask"] = 1

# Interspersed cross-validation setup
ant_data = add_interspersed_folds(ant_data, n_folds=n_fold, seed=42)
...
if __name__ == "__main__":
    ...
    # results = []
    #
    # save_dir = './Results/NN_Results/hyper_rnn/ant'
    ...
    #         train_df = make_full_sequence_split_df(ant_data, test_fold=test_fold, val_fold=val_fold, split="train")
    #         val_df = make_full_sequence_split_df(ant_data, test_fold=test_fold, val_fold=val_fold, split="val")
    #         test_df = make_full_sequence_split_df(ant_data, test_fold=test_fold, val_fold=val_fold, split="test")
    #
    #         mask_total = (train_df["loss_mask"].sum() + val_df["loss_mask"].sum() + test_df["loss_mask"].sum())
    #         assert mask_total == ant_data["choice_mask"].sum()
    #
    ...
    #         )
    #
    #         save_path = (f"{save_dir}/ant_hyper_rnn_fold{test_fold}_{participant_emb_dim}pdim_{hyper_hidden_dim}hyperh_{hidden_dim}hidden.pt")
    #
    ...
    # all_results_df.to_csv(f"{save_dir}/ant_hyper_rnn_interspersed_cv_all_results.csv", index=False)
    #
    ...
    #     ).reset_index())
    #
    # summary_df.to_csv(f"{save_dir}/ant_hyper_rnn_interspersed_cv_summary.csv", index=False)
    ...
    # print(best_overall)
    #
    # pd.DataFrame([best_overall]).to_csv(f"{save_dir}/ant_hyper_rnn_best_interspersed_cv_model.csv", index=False)
    #
    # # ======================================================================================================================
    # # Model Training (Just reuse the code and change the model initialization to fit basic RNN and LSTM)
    # # ======================================================================================================================
    ...
    # results = []
    #
    # save_dir = './Results/NN_Results/rnn/ant'
    # os.makedirs(save_dir, exist_ok=True)
    #
    ...
    #         )
    #
    #         save_path = (f"{save_dir}/ant_rnn_fold{test_fold}_{num_layer}layer_{hidden_dim}hidden.pt")
    #
    #         model, history, best_val = fit_model(
    #             model=model,
    #             train_loader=train_loader,
    ...
    # # Save the results
    # all_results_df = pd.DataFrame(all_results)
    # all_results_df.to_csv(f"{save_dir}/ant_rnn_interspersed_cv_all_results.csv", index=False)
    #
    ...
    #     ).reset_index())
    #
    # summary_df.to_csv(f"{save_dir}/ant_rnn_interspersed_cv_summary.csv", index=False)
    # best_overall = summary_df.sort_values(by="mean_best_val_loss", ascending=True).iloc[0]
    #
    # print("=" * 80)
    # print("Best overall model by mean validation loss:")
    # print(best_overall)
    #
    # pd.DataFrame([best_overall]).to_csv(f"{save_dir}/ant_rnn_best_interspersed_cv_model.csv", index=False)
    ...
    # Define train, val, test splits based on folds
    train_df = ant_data.copy()
    train_df["loss_mask"] = True
    val_df = make_full_sequence_split_df(ant_data, test_fold=None, val_fold=val_fold, split="val")
    ...
    model = HyperRNN(n_participants=n_participants, input_dim=len(ant_cat_col), hidden_dim=final_hidden_dim,
                     output_dim=2, participant_emb_dim=final_participant_emb_dim, hyper_hidden_dim=final_hyper_hidden_dim)

    save_path = f"./Results/Final_Models/ant_hyper_rnn_final_model.pt"
    ...
    np.save(f"./Results/Final_Models/ant_hyper_rnn_final_participant_embeddings.npy", participant_embeddings)
    ...
    #     train_df = make_full_sequence_split_df(ant_data, test_fold=None, val_fold=val_fold, split="train")
    #     val_df = make_full_sequence_split_df(ant_data, test_fold=None, val_fold=val_fold, split="val")
    #
    ...
    #         f"train + val = {mask_total}, total = {ant_data['choice_mask'].sum()}"
    #     )
    #
    ...
    #     save_path = os.path.join(save_dir, f"ant_hyper_rnn_final_model_fold{val_fold}.pt")
    #
    #     model, history, best_val = fit_model(
    #         model=model,
    #         train_loader=train_loader,
    ...
    #     np.save(os.path.join(save_dir, f"ant_hyper_rnn_participant_embeddings_fold{val_fold}.npy"), participant_embeddings)
    #
    ...
    # np.save(os.path.join(save_dir, "ant_hyper_rnn_aligned_fold_embeddings.npy"), aligned_fold_embeddings)
    #
    # # Save aligned average embeddings
    # np.save(os.path.join(save_dir, "ant_hyper_rnn_avg_participant_embeddings_sign_aligned.npy"), avg_participant_embeddings)
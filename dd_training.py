...
#     print(col)
#     print(dd_data[col].value_counts())

# Reassign participant IDs to be consecutive integers starting from 0
dd_data["participant_id"] = dd_data["worker_id"].astype("category").cat.codes
n_participants = dd_data["participant_id"].nunique()
print(n_participants)

# Add trial index
dd_data["trial"] = dd_data.groupby("participant_id").cumcount() + 1
dd_data = dd_data.sort_values(["participant_id", "trial"]).reset_index(drop=True)

# Encode categorical variables
# (this dataset does not have more than 2 categories for each variable, so we can simply use binary encoding)
dd_num_col = ['large_amount', 'later_delay', 'small_amount']
y_col = 'choice'

# Standardize the numerical columns
scaler = MinMaxScaler()
dd_data[dd_num_col] = scaler.fit_transform(dd_data[dd_num_col])

# Make sure y is integer 0/1
dd_data['choice'] = dd_data['choice'].astype('category').cat.codes - 1

# Create choice mask
dd_data["choice_mask"] = (dd_data["choice"] != -1).astype(int)

# Calculate baseline accuracy and NLL
baseline_acc = dd_data['choice'].mean()
print(baseline_acc)
baseline_nll = -(dd_data['choice'] * np.log(baseline_acc) + (1 - dd_data['choice']) * np.log(1 - baseline_acc)).mean()
print(baseline_nll)

# Interspersed cross-validation setup
dd_data = add_interspersed_folds(dd_data, n_folds=n_fold, seed=42)
...
#     results = []
#
#     save_dir = './Results/NN_Results/hyper_rnn/dd'
...
#             train_df = make_full_sequence_split_df(dd_data, test_fold=test_fold, val_fold=val_fold, split="train")
#             val_df = make_full_sequence_split_df(dd_data, test_fold=test_fold, val_fold=val_fold, split="val")
#             test_df = make_full_sequence_split_df(dd_data, test_fold=test_fold, val_fold=val_fold, split="test")
#
#             mask_total = (train_df["loss_mask"].sum() + val_df["loss_mask"].sum() + test_df["loss_mask"].sum())
#             assert mask_total == dd_data["choice_mask"].sum()
#
#             # Transform into sequences
#             train_seq = BehavioralDataset(train_df, x_var=dd_num_col, y_var=y_col)
#             val_seq = BehavioralDataset(val_df, x_var=dd_num_col, y_var=y_col)
#             test_seq = BehavioralDataset(test_df, x_var=dd_num_col, y_var=y_col)
#
#             # Dataloaders
...
#             )
#
#             save_path = (f"{save_dir}/dd_hyper_rnn_fold{test_fold}_{participant_emb_dim}pdim_{hyper_hidden_dim}hyperh_{hidden_dim}hidden.pt")
#
...
#     all_results_df.to_csv(f"{save_dir}/dd_hyper_rnn_interspersed_cv_all_results.csv", index=False)
#
...
#         ).reset_index())
#
#     summary_df.to_csv(f"{save_dir}/dd_hyper_rnn_interspersed_cv_summary.csv", index=False)
...
#     print(best_overall)
#
#     pd.DataFrame([best_overall]).to_csv(f"{save_dir}/dd_hyper_rnn_best_interspersed_cv_model.csv", index=False)
#
#     # ======================================================================================================================
#     # Model Training (Just reuse the code and change the model initialization to fit basic RNN and LSTM)
#     # ======================================================================================================================
...
#     results = []
#
#     save_dir = './Results/NN_Results/rnn/dd'
#     os.makedirs(save_dir, exist_ok=True)
#
...
#             )
#
#             save_path = (f"{save_dir}/dd_rnn_fold{test_fold}_{num_layer}layer_{hidden_dim}hidden.pt")
#
#             model, history, best_val = fit_model(
#                 model=model,
#                 train_loader=train_loader,
...
#     all_results_df = pd.DataFrame(all_results)
#
#     all_results_df.to_csv(f"{save_dir}/dd_rnn_interspersed_cv_all_results.csv", index=False)
#
...
#         ).reset_index())
#
#     summary_df.to_csv(f"{save_dir}/dd_rnn_interspersed_cv_summary.csv", index=False)
#
#     best_overall = summary_df.sort_values(by="mean_best_val_loss", ascending=True).iloc[0]
#
#     print("=" * 80)
...
#     print(best_overall)
#
#     pd.DataFrame([best_overall]).to_csv(f"{save_dir}/dd_rnn_best_interspersed_cv_model.csv", index=False)
    ...
    # Define train, val, test splits based on folds
    train_df = dd_data.copy()
    train_df["loss_mask"] = train_df["choice_mask"]
    val_df = make_full_sequence_split_df(dd_data, test_fold=None, val_fold=val_fold, split="val")
    ...
    model = HyperRNN(n_participants=n_participants, input_dim=len(dd_num_col), hidden_dim=final_hidden_dim,
                     output_dim=2, participant_emb_dim=final_participant_emb_dim, hyper_hidden_dim=final_hyper_hidden_dim)

    save_path = f"./Results/Final_Models/dd_hyper_rnn_final_model.pt"
    ...
    np.save(f"./Results/Final_Models/dd_hyper_rnn_final_participant_embeddings.npy", participant_embeddings)
    ...
    #     train_df = make_full_sequence_split_df(dd_data, test_fold=None, val_fold=val_fold, split="train")
    #     val_df = make_full_sequence_split_df(dd_data, test_fold=None, val_fold=val_fold, split="val")
    #
    ...
    #         f"train + val = {mask_total}, total = {dd_data['choice_mask'].sum()}"
    #     )
    #
    ...
    #     save_path = os.path.join(save_dir, f"dd_hyper_rnn_final_model_fold{val_fold}.pt")
    #
    #     model, history, best_val = fit_model(
    #         model=model,
    #         train_loader=train_loader,
    ...
    #     np.save(os.path.join(save_dir, f"dd_hyper_rnn_participant_embeddings_fold{val_fold}.npy"), participant_embeddings)
    #
    ...
    # np.save(os.path.join(save_dir, "dd_hyper_rnn_aligned_fold_embeddings.npy"), aligned_fold_embeddings)
    #
    # # Save aligned average embeddings
    # np.save(os.path.join(save_dir, "dd_hyper_rnn_avg_participant_embeddings_sign_aligned.npy"), avg_participant_embeddings)

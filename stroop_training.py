...
# # Inspect the data
# for col in stroop_data.columns:
#     print(col)
#     print(stroop_data[col].value_counts())

# Reassign participant IDs to be consecutive integers starting from 0
stroop_data["participant_id"] = stroop_data["worker_id"].astype("category").cat.codes
n_participants = stroop_data["participant_id"].nunique()
print(n_participants)

# Add trial index
stroop_data["trial"] = stroop_data.groupby("participant_id").cumcount() + 1
stroop_data = stroop_data.sort_values(["participant_id", "trial"]).reset_index(drop=True)

# One-hot encoding (3 catgeories)
stroop_data = pd.get_dummies(stroop_data, columns=['stim_color', 'stim_word'], dtype=int)

# Encode
x_col = ['stim_color_blue', 'stim_color_green', 'stim_color_red', 'stim_word_blue', 'stim_word_green', 'stim_word_red', 'trial_type']
y_col = 'correct'

for col in x_col:
    stroop_data[col] = stroop_data[col].astype("category").cat.codes.astype("float32")

# Make sure y is integer 0/1
stroop_data[y_col] = stroop_data[y_col].astype(int)

# Create choice mask
stroop_data['choice_mask'] = (stroop_data['key_press'] != -1).astype(int)

baseline_acc = stroop_data[y_col].mean()
print(baseline_acc)
baseline_nll = -(stroop_data[y_col] * np.log(baseline_acc) + (1 - stroop_data[y_col]) * np.log(1 - baseline_acc)).mean()
print(baseline_nll)

# Interspersed cross-validation setup
stroop_data = add_interspersed_folds(stroop_data, n_folds=n_fold, seed=42)
...
if __name__ == "__main__":
    ...
    # results = []
    #
    # save_dir = './Results/NN_Results/hyper_rnn/stroop'
    ...
    #         train_df = make_full_sequence_split_df(stroop_data, test_fold=test_fold, val_fold=val_fold, split="train")
    #         val_df = make_full_sequence_split_df(stroop_data, test_fold=test_fold, val_fold=val_fold, split="val")
    #         test_df = make_full_sequence_split_df(stroop_data, test_fold=test_fold, val_fold=val_fold, split="test")
    #
    #         mask_total = (train_df["loss_mask"].sum() + val_df["loss_mask"].sum() + test_df["loss_mask"].sum())
    #         assert mask_total == stroop_data["choice_mask"].sum()
    #
    ...
    #         )
    #
    #         save_path = (f"{save_dir}/stroop_hyper_rnn_fold{test_fold}_{participant_emb_dim}pdim_{hyper_hidden_dim}hyperh_{hidden_dim}hidden.pt")
    #
    ...
    # all_results_df.to_csv(f"{save_dir}/stroop_hyper_rnn_interspersed_cv_all_results.csv", index=False)
    #
    ...
    #     ).reset_index())
    #
    # summary_df.to_csv(f"{save_dir}/stroop_hyper_rnn_interspersed_cv_summary.csv", index=False)
    ...
    # print(best_overall)
    #
    # pd.DataFrame([best_overall]).to_csv(f"{save_dir}/stroop_hyper_rnn_best_interspersed_cv_model.csv", index=False)
    #
    # # ======================================================================================================================
    # # Model Training (Just reuse the code and change the model initialization to fit basic RNN and LSTM)
    # # ======================================================================================================================
    ...
    # results = []
    #
    # save_dir = './Results/NN_Results/rnn/stroop'
    # os.makedirs(save_dir, exist_ok=True)
    #
    ...
    #         )
    #
    #         save_path = (f"{save_dir}/stroop_rnn_fold{test_fold}_{num_layer}layer_{hidden_dim}hidden.pt")
    #
    #         model, history, best_val = fit_model(
    #             model=model,
    #             train_loader=train_loader,
    ...
    # # Save the results
    # all_results_df = pd.DataFrame(all_results)
    # all_results_df.to_csv(f"{save_dir}/stroop_rnn_interspersed_cv_all_results.csv", index=False)
    #
    ...
    #     ).reset_index())
    #
    # summary_df.to_csv(f"{save_dir}/stroop_rnn_interspersed_cv_summary.csv", index=False)
    # best_overall = summary_df.sort_values(by="mean_best_val_loss", ascending=True).iloc[0]
    #
    # print("=" * 80)
    # print("Best overall model by mean validation loss:")
    # print(best_overall)
    #
    # pd.DataFrame([best_overall]).to_csv(f"{save_dir}/stroop_rnn_best_interspersed_cv_model.csv", index=False)
    ...
    # Define train, val, test splits based on folds
    train_df = stroop_data.copy()
    train_df["loss_mask"] = train_df["choice_mask"]
    val_df = make_full_sequence_split_df(stroop_data, test_fold=None, val_fold=val_fold, split="val")
    ...
    save_path = f"./Results/Final_Models/stroop_hyper_rnn_final_model.pt"
    ...
    np.save(f"./Results/Final_Models/stroop_hyper_rnn_final_participant_embeddings.npy", participant_embeddings)
    ...
    #     train_df = make_full_sequence_split_df(stroop_data, test_fold=None, val_fold=val_fold, split="train")
    #     val_df = make_full_sequence_split_df(stroop_data, test_fold=None, val_fold=val_fold, split="val")
    #
    ...
    #         f"train + val = {mask_total}, total = {stroop_data['choice_mask'].sum()}"
    #     )
    #
    ...
    #     save_path = os.path.join(save_dir, f"stroop_hyper_rnn_final_model_fold{val_fold}.pt")
    #
    #     model, history, best_val = fit_model(
    #         model=model,
    #         train_loader=train_loader,
    ...
    #     np.save(os.path.join(save_dir, f"stroop_hyper_rnn_participant_embeddings_fold{val_fold}.npy"), participant_embeddings)
    #
    ...
    # np.save(os.path.join(save_dir, "stroop_hyper_rnn_aligned_fold_embeddings.npy"), aligned_fold_embeddings)
    #
    # # Save aligned average embeddings
    # np.save(os.path.join(save_dir, "stroop_hyper_rnn_avg_participant_embeddings_sign_aligned.npy"), avg_participant_embeddings)
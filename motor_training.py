...
# # Inspect the data
# for col in motor_data.columns:
#     print(col)
#     print(motor_data[col].value_counts())

# Reassign participant IDs to be consecutive integers starting from 0
motor_data["participant_id"] = motor_data["worker_id"].astype("category").cat.codes
n_participants = motor_data["participant_id"].nunique()

# Add trial index
motor_data["trial"] = motor_data.groupby("participant_id").cumcount() + 1
motor_data = motor_data.sort_values(["participant_id", "trial"]).reset_index(drop=True)

# One-hot encoding (5 catgeories)
motor_data = pd.get_dummies(motor_data, columns=['trial_condition'], dtype=int)

# Encode
x_col = ['SS_delay', 'trial_condition_crit_go', 'trial_condition_crit_stop', 'trial_condition_noncrit_nosignal',
         'trial_condition_noncrit_signal', 'correct_response']
y_col = 'correct'
cat_col = x_col.copy()
cat_col.remove('SS_delay')

for col in cat_col:
    motor_data[col] = motor_data[col].astype("category").cat.codes.astype("float32")

# Make sure y is integer 0/1
motor_data[y_col] = motor_data[y_col].astype(int)

# Create choice mask
motor_data['choice_mask'] = 1

baseline_acc = motor_data[y_col].mean()
print(baseline_acc)
baseline_nll = -(motor_data[y_col] * np.log(baseline_acc) + (1 - motor_data[y_col]) * np.log(1 - baseline_acc)).mean()
print(baseline_nll)

# Interspersed cross-validation setup
motor_data = add_interspersed_folds(motor_data, n_folds=n_fold, seed=42)
...
if __name__ == "__main__":
    ...
    # results = []
    #
    # save_dir = './Results/NN_Results/hyper_rnn/motor'
    ...
    #         train_df = make_full_sequence_split_df(motor_data, test_fold=test_fold, val_fold=val_fold, split="train")
    #         val_df = make_full_sequence_split_df(motor_data, test_fold=test_fold, val_fold=val_fold, split="val")
    #         test_df = make_full_sequence_split_df(motor_data, test_fold=test_fold, val_fold=val_fold, split="test")
    #
    #         mask_total = (train_df["loss_mask"].sum() + val_df["loss_mask"].sum() + test_df["loss_mask"].sum())
    #         assert mask_total == motor_data["choice_mask"].sum()
    #
    ...
    #         )
    #
    #         save_path = (f"{save_dir}/motor_hyper_rnn_fold{test_fold}_{participant_emb_dim}pdim_{hyper_hidden_dim}hyperh_{hidden_dim}hidden.pt")
    #
    #         # check if the model already exists
    #         if os.path.exists(save_path):
    #             print(f"Model already exists at {save_path}, loading the model.")
    #             model.load_state_dict(torch.load(save_path))
    #             model.to(device)
    #             history = None  # History is not available when loading a model
    #             best_val = None  # Best val loss is not available when loading a model
    #         else:
    ...
    # all_results_df.to_csv(f"{save_dir}/motor_hyper_rnn_interspersed_cv_all_results.csv", index=False)
    #
    ...
    #     ).reset_index())
    #
    # summary_df.to_csv(f"{save_dir}/motor_hyper_rnn_interspersed_cv_summary.csv", index=False)
    ...
    # print(best_overall)
    #
    # pd.DataFrame([best_overall]).to_csv(f"{save_dir}/motor_hyper_rnn_best_interspersed_cv_model.csv", index=False)
    ...
    # results = []
    #
    # save_dir = './Results/NN_Results/rnn/motor'
    # os.makedirs(save_dir, exist_ok=True)
    #
    ...
    #         )
    #
    #         save_path = (f"{save_dir}/motor_rnn_fold{test_fold}_{num_layer}layer_{hidden_dim}hidden.pt")
    #
    #         model, history, best_val = fit_model(
    #             model=model,
    #             train_loader=train_loader,
    ...
    # # Save the results
    # all_results_df = pd.DataFrame(all_results)
    # all_results_df.to_csv(f"{save_dir}/motor_rnn_interspersed_cv_all_results.csv", index=False)
    #
    ...
    #     ).reset_index())
    #
    # summary_df.to_csv(f"{save_dir}/motor_rnn_interspersed_cv_summary.csv", index=False)
    # best_overall = summary_df.sort_values(by="mean_best_val_loss", ascending=True).iloc[0]
    #
    # print("=" * 80)
    # print("Best overall model by mean validation loss:")
    # print(best_overall)
    #
    # pd.DataFrame([best_overall]).to_csv(f"{save_dir}/motor_rnn_best_interspersed_cv_model.csv", index=False)
    ...
    # Define train, val, test splits based on folds
    train_df = motor_data.copy()
    train_df["loss_mask"] = train_df["choice_mask"]
    val_df = make_full_sequence_split_df(motor_data, test_fold=None, val_fold=val_fold, split="val")
    ...
    save_path = f"./Results/Final_Models/motor_hyper_rnn_final_model.pt"
    ...
    y_true, y_pred, p_correct = collect_predictions(model, train_loader, device)
    y_true = np.asarray(y_true)
    y_pred = np.asarray(y_pred)
    p_correct = np.asarray(p_correct)
    test_acc = (y_true == y_pred).mean()
    ...
    np.save(f"./Results/Final_Models/motor_hyper_rnn_final_participant_embeddings.npy", participant_embeddings)
    ...
    #     train_df = make_full_sequence_split_df(motor_data, test_fold=None, val_fold=val_fold, split="train")
    #     val_df = make_full_sequence_split_df(motor_data, test_fold=None, val_fold=val_fold, split="val")
    #
    ...
    #         f"train + val = {mask_total}, total = {motor_data['choice_mask'].sum()}"
    #     )
    #
    ...
    #     save_path = os.path.join(save_dir, f"motor_hyper_rnn_final_model_fold{val_fold}.pt")
    #
    #     model, history, best_val = fit_model(
    #         model=model,
    #         train_loader=train_loader,
    ...
    #     np.save(os.path.join(save_dir, f"motor_hyper_rnn_participant_embeddings_fold{val_fold}.npy"), participant_embeddings)
    #
    ...
    # np.save(os.path.join(save_dir, "motor_hyper_rnn_aligned_fold_embeddings.npy"), aligned_fold_embeddings)
    #
    # # Save aligned average embeddings
    # np.save(os.path.join(save_dir, "motor_hyper_rnn_avg_participant_embeddings_sign_aligned.npy"), avg_participant_embeddings)